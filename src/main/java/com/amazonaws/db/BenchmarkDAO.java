...
public class BenchmarkDAO{ 
    ...
    public Benchmark getBenchmark(String name) throws Exception {
        ...
        try {
            ...
            while (resultSet.next()){
                bench = new Benchmark(resultSet.getString("UID"),resultSet.getDate("date"),resultSet.getString("name"),idao.getImplementationByID(resultSet.getString("implemenation")),mdao.getMachineConfig(resultSet.getString("machine_config")),pdao.getProblemInstance(resultSet.getString("problem_instance")),resultSet.getDouble("runtime"),resultSet.getString("observation"));
            }
            resultSet.close();
            ps.close();
    ...
    public boolean addBenchmark(Benchmark bench) throws Exception {
    	...
    	try {
            ...
            ps = conn.prepareStatement("INSERT INTO " + tblName + " (UID,date,runtime,observations,machine_config,implementation,problem_instance) values(UUID(),CURDATE(),?,?,?,?,?);");
